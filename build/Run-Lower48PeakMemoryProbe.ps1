...
"@
$csproj = $csproj.Replace('$RepoRoot', $RepoRoot)
...
$report = [ordered]@{
  ...
  config = [ordered]@{ maxDopRequested = $MaxDop; memoryBudgetGiB = $MemoryBudgetGiB; scratchBudgetGiB = $ScratchBudgetGiB; intermediateStorage = 'MemoryMapped'; gridDivisions = 8; hierarchy = $false; shortcuts = $false }
  runs = $runs
  ...
  verdict = [ordered]@{ status = $(if($anyOk){'lab-capacity-measurement-success'}else{'lab-capacity-measurement-failed'}); formalBarMet = ($cpu -ge 32 -and $memGiB -ge 64); formalPass = $false; notes = 'Lab peak-memory measurement for GCP sizing. Formal 32/64 bar not claimed.' }
